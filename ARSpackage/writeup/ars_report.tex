...
\subsection*{ eval\_h method }
In this method, we calculate the values of $h(x)$ and $h'(x)$ for any $x$.

\subsection*{ s\_x method }
In this method, we calculate $s_{k}(x)$ by normalizing $e^{ u_{k}(x)}$.

\subsection*{ sample method }
In this method, we sample a new $x^{*}$ from the $s_{k}(x)$ by using inverse cdf of  $s_{k}(x)$ with a random value $u \sim unif(0,1)$.

\subsection*{ upper method }
In this method, we calculate the $u_{k}(x)$ for all $x$'s.


\subsection*{ lower method }
In this method, we calculate the $l_{k}(x)$ for all $x$'s.


\subsection*{ update method }
In this method, we test the sample element $x^{*}$ from sample method. Once it is accepted, we add the $x^{*}$ into the $x$ set.
